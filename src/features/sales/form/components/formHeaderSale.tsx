        ...
        <div
          className={`bar absolute transition-all duration-500 ${
            formSteps == 0
              ...
              ? "w-[63%]"
              : formSteps == 3 && "w-full"
          } h-[2px] bg-primary top-[28%] translate-y-[-50%]`}
        ></div>
        ...
        {sectionArray.map((item, index) => (
          <div key={index} className="relative z-30">
            <div className="flex justify-center">
              <span
                className={`flex justify-center bg-white items-center rounded-full h-[30px] w-[30px] border-2 ${
                  formSteps >= index
            ...
            <p
              className={`font-open_sans  font-medium mt-1 ${
                formSteps >= index ? "text-primary-dark" : "text-gray-400"
              }`}
            >
              {item}
            </p>
