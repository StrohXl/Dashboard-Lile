...
export default function Invoice({
  ...
  return (
    <div ref={containerInvoice} className="font-roboto text-sm p-6 text-black">
      ...
      <div className="pb-2 border-b-1 mb-2 border-gray-300">
        <div className="grid grid-cols-[70%_1fr] gap-4 mb-2">
          <h4 className="uppercase">Producto</h4>
          <h4 className="uppercase text-end">Precio</h4>
        </div>
      ...
      <div className="pb-2 border-b-1 mb-2 border-gray-300">
        <div className="grid grid-cols-[40%_1fr_1fr] gap-4 mb-2">
          <h4 className="uppercase">Metodo</h4>
          <h4 className="uppercase">Monto</h4>
          <h4 className="uppercase text-end">Operacion</h4>
        </div>
        {payments &&
          payments.map((item, index) => {
            return (
              <div key={index} className="grid grid-cols-[40%_1fr_1fr] gap-4">
                <h4 className="uppercase">{item.payment_method}</h4>
                <h4>
                  {item.payment_method == "divisa" ? "$ " : "Bs "}
                  {item.payment_amount}
                </h4>
                <h4 className="text-end">
                  {item.payment_method == "transferencia" && item.operation}
                </h4>
              </div>
            );
          })}
